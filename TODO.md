...
- ⏭️ **Recommended next:** the **nit-transparency fix** (additive, no spec conflict) — `-F`/stdin commit body + stop clap eating `-- <pathspec>` for `nit diff/log/status` (observed-behavior #5). Deliberately NOT bundled into the keystone (Co-triage / commit-caution). Additive + not safety-critical `cmd_commit` surgery → not marathon-tail-sensitive.
...
- [ ] **Index advisory lock — DEMOTED, still unneeded.** Session-intent scoping (landed this commit) fully neutralizes the race (verified end-to-end against the `3cf94eb8` simulation). Revisit ONLY if a real-world failure proves the keystone insufficient.
...
**Recommended order (updated 2026-05-17):** ✅ clippy hygiene (`51e002a`) → ✅ **honor-staged-scope keystone + re-scope-deploy-to-staged via Session-intent scoping** (this commit; RED-GREEN + end-to-end verified) → ⏭️ **nit-transparency fix** (next; additive, not marathon-sensitive) → index lock only if a real failure proves the keystone insufficient (currently unneeded) → then a deliberate `semikolon/nit` push decision (still NOTHING pushed; all local).
