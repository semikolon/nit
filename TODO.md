...
- 🔴 **PRE-PUSH BLOCKER:** 11 PRE-EXISTING clippy errors (encrypt.rs ×4, syncbase.rs ×4, trigger.rs ×2, one existing config.rs test at :893) — local Rust 1.93 is stricter than nit's last-clean CI baseline. **NOT introduced by the forward-only work; deliberately NOT swept-fixed** (bundling unrelated cleanup into a feature commit = the Co-triage / commit-caution anti-pattern). Must be cleared in a SEPARATE scoped commit before ANY `semikolon/nit` push.
...
- 🔴 **Clear the pre-existing clippy toolchain-drift** (separate scoped commit, BEFORE any `semikolon/nit` push). See Status block. Do NOT bundle into a feature commit.
