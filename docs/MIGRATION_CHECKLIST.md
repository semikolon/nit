...
- [x] **Forward-only sync semantics for append-only runtime data** (Apr 20, 2026 — ✅ IMPLEMENTED nit `3d6bfec`, **PUSHED to `semikolon/nit` 2026-05-17** in `b26cf95`; Mac Mini running the hardened binary; fleet auto-catches-up on next `nit update` via `.nit-version`=`b26cf95`. **Remaining activation gate (deferred dotfiles pass):** declare the 3 runtime paths in `fleet.toml [sync] forward_only`. **Clippy-blocker reframe:** the "pre-push clippy blocker" was a mis-frame — `cargo clippy -- -D warnings` (the real CI gate) was already green; the 11 were `--all-targets` test-only lints, cleared in `51e002a`. SSoT = project `TODO.md` § Status; design below retained as the implemented spec): chezmoi's source-wins-on-apply is the wrong model for files primarily *written* by runtime processes rather than *edited* by humans. Today this works only because `chezmoi-auto-apply` is disabled — a stray `chezmoi apply --force <target>` silently overwrites newer runtime state with the older source snapshot, losing content between the last `re-add` and now. Affected files include:
  ...
  **Proposed systemic solution for nit** — extend the per-file classification system with a new class:
  ...
  > **⚠ Shipped reality (2026-05-18, this commit) — supersedes "Flush … commits" above.** `nit sync` no longer git-commits. A commit on `master` rides the push lineage onto origin and then merge-conflicts every fleet machine's `git pull` of its *own* runtime-drifted copy (caught by dogfooding this session — a `nit sync` snapshot commit entangled a `.nit-version` deploy + a concurrent session's commit stacked on it, making soft-reset-out unsafe). `nit sync` now atomically snapshots each present forward-only path to `~/.local/share/nit/forward-only/` (restic-covered, same tier as sync-base/drift) — local-only, never a commit, never pushed; origin's forward-only files stay static so bootstrap still seeds from that baseline. RED-GREEN `present_forward_only` + suite green (212) + e2e-smoked. **Residual per-host-propagation caveat:** the pre-fix snapshot commits made earlier this session are ALREADY on origin; before a fleet machine's first post-upgrade pull, drift-resolve its forward-only paths (snapshot-then-`git checkout -- <paths>`) so that one historical pull can't merge-conflict — one-time, folds into the per-host propagation playbook.
...
- **Phantom-drift lifecycle bug — FIXED (part 1), part 2 deferred (May 16, 2026)**: `save_drift()` had no lifecycle pair. When a drift was resolved via the *recommended* "edit the template source" path, the `~/.local/share/nit/drift/<rel>.diff` artifact was orphaned forever. `list_drifted_files()` is presence-based, so `nit status` reported phantom drift indefinitely (real incident: `~/.zshenv.diff` stale 2026-05-04→05-16, ~12 days, after the bw-latency fix was incorporated into `templates/.zshenv.tmpl` but the artifact never cleaned; `cmd_apply` already recomputed "0 drifted" correctly — only the stored artifact lied). **Fix shipped (nit commit `5e48b63`; fleet-rolled 2026-05-17 — pushed to `semikolon/nit`, `.nit-version`→`b26cf95`, Mac Mini running the new binary via `nit run rebuild-nit`, fleet auto-catches-up next `nit update`):** new `syncbase::clear_drift()` (idempotent, infallible — distinct from `dismiss_drift`), called on every clean (no-real-drift) deploy branch in `cmd_apply`+`cmd_update`, where `target == sync-base` so any saved `.diff` is provably stale. A drift resolved out-of-band now self-heals on the next apply/update of that file. 2 tests; full suite 182 green; fmt+clippy clean. **Sacred drift-safety preserved** (real drift still detected/saved/skipped — only the provably-stale artifact in the no-drift branch is cleared). **Deferred (tracked, dotfiles TODO):** *part 2* — make `nit status`/`nit update` pre-check RECOMPUTE drift instead of trusting `list_drifted_files()` presence (intentionally not done: modifies the safety-critical pre-pull abort path that correctly prevented a 2026-05-04 clobber; part 1 self-heals on next apply so marginal value is low). *Fleet rollout* — ✅ DONE 2026-05-17: pushed `semikolon/nit`, `~/dotfiles/.nit-version`→`b26cf95`, Mac Mini upgraded via `nit run rebuild-nit`; other fleet machines auto-catch-up on next `nit update`. (Only *part 2* remains deferred — sound: don't refactor the safety-critical pre-pull abort for a low marginal gain.) NOTE: the May-15 nightly `nit update` "ABORTED (drift)" was a SEPARATE issue (pre-pull GIT drift on a tracked file, `detect_pre_pull_drift`), not this artifact — `git status` was clean by 05-16 so that self-resolved; the template-drift loop only *skips* drifted templates, it never aborts the update.

- **Known non-issue + keystone cross-ref (2026-05-17):** (a) The shared fleet-wide pre-commit hook (`~/.git-templates/hooks/pre-commit`) flags nit's *test-fixture* secret-like strings (`encrypt.rs` `API_KEY=secret123`, `:/` pathspec literals, `secret`/`token` substrings) on every commit touching those tests. **Non-blocking** (commits succeed) and **trufflehog is authoritative** (0 verified / 0 findings of any kind across all pushed commits, verified 2026-05-17). Do NOT unilaterally loosen the shared hook regex — it guards every fleet repo; a too-loose pattern could leak a real secret fleet-wide (*Consult before shared-infrastructure design changes*). Tune-vs-accept is a deliberate user decision; default = accept the cosmetic noise. (b) The **concurrent-session commit-safety keystone** shipped + pushed in `b26cf95`: `nit commit` is now session-intent-scoped (commits only what THIS session staged, never a concurrent session's shared-index work) and `nit status -v` shows a pre-commit scope preview. Full detail + RED-GREEN + the archaeology-reframe correction: project `TODO.md` § "Concurrent-session commit safety".
