...
- [x] **Forward-only sync semantics for append-only runtime data** (Apr 20, 2026 — ✅ IMPLEMENTED 2026-05-17, nit `3d6bfec` LOCAL/not-pushed; declared via `[sync] forward_only = [paths]` in fleet.toml; SSoT = project `TODO.md` § Status incl. activation gates + the pre-push clippy blocker; design below retained as the implemented spec): chezmoi's source-wins-on-apply is the wrong model for files primarily *written* by runtime processes rather than *edited* by humans. Today this works only because `chezmoi-auto-apply` is disabled — a stray `chezmoi apply --force <target>` silently overwrites newer runtime state with the older source snapshot, losing content between the last `re-add` and now. Affected files include:
